 ...
 */
 // 
   g=(d-1)/2;
   out=(unsigned char *)malloc(starter[0]*starter[1]);
   if(!out)
   {
 ...
 fwrite(starter,sizeof(unsigned int),3,fid);
 fseek(fid,8l,SEEK_CUR);
 fwrite(out,1,starter[0]*starter[1],fid);
...
w=0;x=0;y=0;z=0;
  ...
  for(i=0;i<starter[0];i++)
   ...
   //	printf("XXX");
	...
	k=-90;
//theta=k+atan(beta/alpha)*180/3.14159265;
	printf("\nCoordinates(appro.): ROW=%d , COLUMN=%d \n alpha :%f beta: %f \n Angle theta : %f degrees",i,j,alpha,beta,theta);
free(out);
fclose(fid);
t4=clock();
printf("\n Timing Info in msec: 1]t3-t2 (Median Filter) :%g\n 2]t4-t1(Including Read-write overheads):%g",(double)(t3-t2)*1000/CLOCKS_PER_SEC,(double)(t4-t1)*1000/CLOCKS_PER_SEC);	
