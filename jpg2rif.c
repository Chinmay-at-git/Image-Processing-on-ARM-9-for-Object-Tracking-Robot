...
//---------------------------
// CHINMAY EDIT 27.12.13  signed by ~CAJ
//~CAJ
#define INFILE "D:\\study\\projectwork_current\\download.jpg"
#define OUTFILE "D:\\study\\projectwork_current\\out.rif"
// END ~CAJ
//---------------------------------------------------
#include "picojpeg.h"
#include "picojpeg.c"
...
*/
//------------------------------------------------------------------------------
unsigned char * jpg2rif(const char *pSrc_filename,int *w,int *h,int reduce)
{
   int n = 1,i=0;
   //const char *pSrc_filename; //:chinmay
   //const char *pDst_filename;//:chinmay
  // argv: first destination(output) then source (input) file
   
   char flag=3;
   unsigned int width, height, comps,str[3]; //~CAJ unsigned
  ...
  // clock_t t1,t2,t3,t4;
   FILE *fid;
   //printf("\n --------------------------------------------------------\n");
   //printf("picojpeg example v1.1, Rich Geldreich <richgel99@gmail.com>,V1.0  \n");
   //printf("\n Modified by Chinmay for jpeg to rif(raw image file) format conversion.\n For Final Y project \nCompiled at " __TIME__ " " __DATE__"\n");
   //printf("\n --------------------------------------------------------\n");
   //if ((argc < 3) || (argc > 4))
     // return print_usage();
   ...
   //pSrc_filename = arg_v[n++]; ~CAJ
   //pDst_filename = arg_v[n++]; ~CAJ
   ...
   if (argc == 4)
   {
      ...
      {
      	//printf("Invalid Reduce argument. Reduce can be 0 or 1.if 1, it quickly decodes image to ~ 1/8 resolution,\n Taking Reduce=0\n" );
      	reduce=0;
      }
}
*/
  ...
  //printf("Width: %i, Height: %i, Comps: %i\n", width, height, comps);
   ...
   switch (scan_type)
   {
      case PJPG_GRAYSCALE: p = "GRAYSCALE"; flag=1; break;
  ...
  /*~CAJ if (!stbi_write_tga(pDst_filename, width, height, comps, pImage))
   {
   ...
   */
   //~CAJ
/*
   fid=fopen(pDst_filename,"wb");
   if(!fid)
   {
   
   //printf("\nOutput File opening problem..... Abort");
   return(1);
}
*/
   //t3=clock();
   ...
   *h=height;
//   str[2]=comps;
  // fwrite(str,sizeof(unsigned int),3,fid);
   //fseek(fid,8l,SEEK_CUR);
 // i=fwrite(pImage,1,width*height*comps,fid);
   //	t4=clock();
   //fclose(fid);
